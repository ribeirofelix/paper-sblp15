...
\title{An Approach for Preserving Lexical Scoping When Embedding Languages}
...
\begin{verbatim}
 ...
 8      void foo () {
 9        int y = 10;
10        Expression <sumY> ret = boo();
11        int res = ret.Compile()(41); // res is 42
12      }
13    }
\end{verbatim}
...
only to produce an expression tree. Expression trees can be compiled to
produce an anonymous function at a different scope, and still they will
reference variables from the point where the expression was created. Figure
...
from line 4, even when the expression tree returned in line 10 is compiled
into a function in line 11.

Terra \cite{DeVito2013Terra} is a multi-stage language for high-performance
computing. It extends Lua, using it as a host language and defines extensions
for staged computation. Lua functions that run in the Lua interpreter are
declared using standard Lua syntax, with the \texttt{function} statement.
...
Any free Lua variables referenced in Terra functions are bound at the time of
their declarations, as are Lua expressions escaped inside Terra functions
using brackets. A Terra function, therefore, does not form a closure with
...
\emph{upvalues}. In Lua, \emph{upvalues} are non-local variables, that is,
...
Metalua, but including information about \emph{upvalues}. While Metalua
