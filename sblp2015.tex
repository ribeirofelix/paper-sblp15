...
As an example, LINQ (Language Integrated-Query) is a set of resources that
...
a data collection.
For instance, consider a list of objects of the class \texttt{PL} that contains
...
If the list is a mapping from a relation data base, then LINQ generated SQL
code to query the data base.
...
However, C\# has a special name for lambda expressions: \texttt{delegate},
that is, a reference type to encapsulate an anonymous function.
...
Since we cannot perform other action over a function besides calling it,
...
lambda expression that should create an AST instead of an anonymous function.
...
For instance, Lisp introduces the operator \texttt{'} to be used before an expression.
When the compiler finds this operator, it does not evaluate the expression,
but it creates an AST for the expression.
...
This example shows that an AST that describes a lambda expression that uses
variables from an external local scope needs the information about the context
where this expression was defined.
When an AST stores more information than the syntax of the written code,
we say that this AST is an annotated AST.

When a list has as its source a relational data base, LINQ uses this mechanism
to during compile-time create an AST that translates the code to SQL.
When this lambda expression is sent to a variable that was declared as
