...
expression trees. Free variable \texttt{y} in line 5 references declaration
...
references to local variables of declared in an outer scope.
...
In Lua, a function variable of outer lexical scope is called upvalues\cite{Ierusalimschy2006PIL} due to historical roots.
% TODO: explain upvalues, \cite{Ierusalimschy2006PIL}

The process of generation uses external libraries: luadec and lua-parser.
...
For example, we can use a user string input in a form of Lua syntax and use \texttt{dostring} in order to execute this input inside a running program.
Due to our method is done at runtime, with no syntax modification and then no need of preprocessors, functions as \texttt{dostring} may still be used.
...
\texttt{toAST} generates an AST from a Lua function.
...
The \texttt{toAST(func)} function calls the function \texttt{luadec(arg)} that belongs to \texttt{libluadec}, sending the function that will be transformed in AST as argument.
...
Later, we localize the free variables of the function.
For each localized variable a code lookup is made with the standard library debug.
The lookup consists in going through the upvalues that belongs to the function that was sent to \texttt{toAST} in order to find the variable of the same name.
...
This function gets an upvalue from a Lua closure and make it refer to the desirable upvalue.
So, in order to use this function, we need create a Lua closure with an upvalue.
To do so, we created the functions that follows:
...
So, when an upvalue is found, the \texttt{newclosure} function is called, then its return is assigned to a variable and the found upvalue is joint to the \texttt{temp} upvalue.
...
In adition, the command \texttt{last.print} dumps the AST in textual format, following the syntax defined by Metalua in \cite{metalua}.
...
% Dica geral: pra deixar as explicações mais claras, evita a voz passiva.
% Escrever na voz ativa te força a dizer _quem_ executa cada ação,
% e é mais fácil pra quem lê acompanhar.
...
% Adaptar o exemplo pra ficar igual ao do LINQ. De repente em vez de ter esses trechos de código inline, seria melhor fazer uma figura como na seção 2.
% O paralelo com o LINQ ficaria mais claro.
